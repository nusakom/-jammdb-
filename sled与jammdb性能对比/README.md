# jammdb与sled性能对比
在现代应用程序中，性能至关重要。本文将展示如何使用 Rust 的 sled 数据库和 Tokio 异步运行时，构建一个高效的数据库解决方案。我们将专注于插入数据的速度和效率。
## 测试环境：
...
### 注意事项
- **线程数量**：线程数量过多可能导致性能下降，因为线程切换的开销也会增加。
- **数据量**：在数据量较小的情况下，同步操作可能不是最优选择，可以考虑使用无锁数据结构或异步编程模型来提高性能。
- **锁竞争**：在极端情况下，大量线程同时竞争同一个锁可能导致性能瓶颈，需要根据实际应用场景进行优化。
---
## 创建一个简单的 Sled 数据库
### 代码实现
```rust
...
jammdb与sled性能对比\屏幕截图 2024-10-02 065410.jpg
```
...
```
完成插入数据，耗时: 15.61s，吞吐量: 64048.72 条/秒
```
