...
2. **性能**：
   - **Future**：由于其非阻塞特性，能够在等待 I/O 操作时让出控制权，提高了并发性能。`jammdb_future` 显示出显著的吞吐量，说明其高效处理大量并发任务。
   - **绿色线程**：虽然也可以处理高并发，但在多线程模型中，资源管理和上下文切换可能导致性能下降。在 `jammdb_green` 中，吞吐量为，相对较低。
3. **编程模型**：
